...
// @brief   A class for representing the columns of a Trellis-object.        //
///////////////////////////////////////////////////////////////////////////////
...
float TrellisColumn::get_emission_score(const ParamTable &pt, 
					unsigned int label_index) const
{
  return pt.get_all_unstruct(*word, word->get_label(label_index));
}
  ...
  for (unsigned int i = 0; i < (ncol == 0 ? 1 : ncol->label_count); ++i)
    {
      ...
      float tr_score = pt.get_all_struct_bw(plabel, label, nlabel);
